...
function getPostSchema(PDO $pdo): array {
  ...
  $hasDocs = in_array('documents', $cols, true);
  $hasSimpozijum = in_array('show_on_simpozijum', $cols, true);
  ...
  if ($hasDocs) $fields[] = 'documents';
  if ($hasSimpozijum) $fields[] = 'show_on_simpozijum';
  ...
  return $state = [
    ...
    'has_show_on_simpozijum' => $hasSimpozijum,
...
function normalizeDocuments($value): array {
  if (is_array($value)) {
    return array_values(array_filter($value, function ($d) {
      return is_array($d) && !empty($d['url']);
    }));
  }
  if (is_string($value) && $value !== '') {
    $decoded = json_decode($value, true);
    if (is_array($decoded)) return normalizeDocuments($decoded);
  }
  return [];
}

function serializeDocuments(array $docs): string {
  return json_encode(array_values($docs), JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
}

function slugifyValue(string $input): string {
  $map = [
...
function buildUniqueSlug(PDO $pdo, string $source): string {
  $baseSlug = slugifyValue($source);
  if ($baseSlug === '') {
    return '';
  }
...
function formatPostRow(array $row, array $schema): array {
  ...
  if ($schema['has_documents']) {
    $row['documents'] = normalizeDocuments($row['documents'] ?? '');
  } else {
    $row['documents'] = [];
  }

  $row['show_on_simpozijum'] = !empty($row['show_on_simpozijum']);
...
if ($method === 'POST') {
  ...
  $slug = buildUniqueSlug($pdo, !empty($input['slug']) ? (string)$input['slug'] : (string)$input['title']);

  if ($slug === '') {
    http_response_code(400);
    echo json_encode(['message'=>'Slug nije validan.'], JSON_UNESCAPED_UNICODE);
    exit;
  }
  ...
  if ($schema['has_documents']) {
    $docs = normalizeDocuments($input['documents'] ?? []);
    $fields[] = 'documents';
    $values[] = serializeDocuments($docs);
  }

  if ($schema['has_show_on_simpozijum']) {
    $fields[] = 'show_on_simpozijum';
    $values[] = !empty($input['showOnSimpozijum']) || !empty($input['show_on_simpozijum']) ? 1 : 0;
  }
...
if ($method === 'PUT') {
  ...
  if ($schema['has_documents'] && array_key_exists('documents', $input)) {
    $docs = normalizeDocuments($input['documents'] ?? []);
    $update['documents'] = serializeDocuments($docs);
  }

  if ($schema['has_show_on_simpozijum'] && (array_key_exists('showOnSimpozijum', $input) || array_key_exists('show_on_simpozijum', $input))) {
    $update['show_on_simpozijum'] = !empty($input['showOnSimpozijum']) || !empty($input['show_on_simpozijum']) ? 1 : 0;
  }
